...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    T mRefreshableView;
    private RefreshableViewWrapperFrameLayout mRefreshableViewWrapper;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                if (mIsBeingDragged) {
                    ...
                    if (!isPullUp && mCurrentMode == Mode.PULL_FROM_START) {
                        setState(State.RESET);
                        return true;
                    } else if (!isPullDown && mCurrentMode == Mode.PULL_FROM_END) {
                        setState(State.RESET);
                        return true;
                    }
    ...
    protected RefreshableViewWrapperFrameLayout getRefreshableViewWrapper() {
        ...
        switch (mCurrentMode) {
            case PULL_FROM_END:
                if (isPullDown) {
                    ...
                    if(mFooterLayout.getVisibility()!=View.INVISIBLE){
    ...
    protected void onReset() {
        ...
        if(isPullDown){
            mFooterLayout.reset();
        }else if(mFooterLayout.getVisibility()!=View.INVISIBLE){
            mFooterLayout.setVisibility(View.INVISIBLE);
        }
        if(isPullUp){
            mHeaderLayout.reset();
        }else if(mHeaderLayout.getVisibility()!=View.INVISIBLE){
            mHeaderLayout.setVisibility(View.INVISIBLE);
        }
    ...
    protected final void setHeaderScroll(int value) {
        ...
        if (mLayoutVisibilityChangesEnabled) {
            if (value < 0) {
                ...
                if (isPullDown && mCurrentMode == Mode.PULL_FROM_END) {
    ...
    private void addRefreshableView(Context context, T refreshableView) {
        mRefreshableViewWrapper = new RefreshableViewWrapperFrameLayout(context);
    ...
    private void pullEvent() {
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            ...
            switch (mCurrentMode) {
                case PULL_FROM_END:
                    if (isPullDown) {//如果是pullUp则显示pull标题信息等
                        if (mFooterLayout.getVisibility() != View.VISIBLE) {
                            mFooterLayout.setVisibility(View.VISIBLE);
                        }
                ...
                case PULL_FROM_START:
                default:
                    if (isPullUp) {//如果是pullUp则显示pull标题信息等
                        if (mHeaderLayout.getVisibility() != View.VISIBLE) {
                            mHeaderLayout.setVisibility(View.VISIBLE);
                        }
    ...
    public void enablePullDown(Boolean isPullDown) {
        this.isPullDown = isPullDown;
    }

    //是否开启pullUp
    public void enablePullUp(Boolean isPullUp) {
